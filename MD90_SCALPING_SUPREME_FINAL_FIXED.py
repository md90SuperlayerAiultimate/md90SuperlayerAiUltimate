...
# ============================================================
# 0. SETUP LOGGING & ENVIRONMENT VALIDATION
# ============================================================

logging.basicConfig(level=logging.INFO, format='%(asctime)s - %(levelname)s - %(message)s')
logger = logging.getLogger(__name__)

# FIX #2: VALIDATE CRITICAL ENV VARS AT STARTUP
...
def get_binance_client():
    ...
    with _client_lock:
        if _client_cache is None:
            ...
            _client_cache = Client(api_key, api_secret)
...
class MarketData:
    ...
    # Real-Time (Baru)
    gex_zero_gamma: float = 0.0      # Level Zero Gamma (GEX)
    gex_score: float = 0.0           # -1.0 s/d +1.0
    cot_managed_money_net_long: int = 0
    cot_bias: str = "neutral"        # "bullish", "bearish", "contrarian_bearish"
...
# ============================================================
# 2. KERANGKA BERPIKIR (Fase Analisis: 20 Layer + 5 Gates)
# ============================================================
...
class ThinkingFramework:
    ...
    def get_all_votes(self) -> List[LayerVote]:
        return [
            ...
            self.layer_2_ema(),
            self.layer_3_rsi(),
            self.layer_4_stochastic(),
            self.layer_5_orderblock(),
            self.layer_6_volume(),
            self.layer_7_mtf(),
            self.layer_8_sentimen(),
            self.layer_9_ultrascalper(),
            self.layer_10_primeswing(),
            self.layer_11_macroflow(),
            self.layer_12_vwap(),
            self.layer_13_atr(),
            self.layer_14_ote_fibonacci(),
            self.layer_15_idm_fvg(),
            self.layer_16_mastercall(),
            self.layer_17_time_stop(),
            self.layer_18_gex(),
            self.layer_19_cot(),
            self.layer_20_orderbook()
        ]

    # -------------------- 5 GATES OF FUSION --------------------
    def validate_gates(self, votes: List[LayerVote]) -> Tuple[bool, TradeAction]:
        """Gate 1-5 berurutan. Jika gagal, langsung STOP."""
        vote_dict = {v.layer_name: v for v in votes}
        
        # Gate 1: Trend & Structure (EMA, MTF, PrimeSwing)
        dir1 = self._get_direction(vote_dict, ["EMA", "MTF", "PrimeSwing"])
        if dir1 == TradeAction.HOLD: return False, TradeAction.HOLD
        
        # Gate 2: Location & Trap (OTE, IDM/FVG, GEX, ORDERBOOK)
        ...
        # Gate 3: Momentum (RSI, Stoch, Volume, VWAP, COT) → minimal 3 dari 5 searah
        ...
        # Gate 4: Trigger (MasterCall + UltraScalper)
...
# ============================================================
# 3. LOGIKA EKSEKUSI (Fase Realisasi: Entry, SL, TP, RR)
# ============================================================
...
class ExecutionLogic:
    ...
    def calculate_entry(self) -> float:
        """Entry dinamis berdasarkan gamma & order book"""
        cp = self.data.current_price
        zero = self.data.gex_zero_gamma
        if self.direction == TradeAction.SELL:
            if cp < zero:  # negative gamma → entry lebih rendah (0.2% di bawah)
                return cp * 0.998
    ...
    def calculate_sl(self, entry: float) -> float:
        """SL = swing terdekat + 0.3 × ATR Live"""
        atr = self.data.atr
        if self.direction == TradeAction.SELL:
    ...
    def calculate_tp_grid(self, entry: float, sl: float) -> Tuple[float, float, float]:
        """TP1, TP2, TP3 berdasarkan Order Book + Liquidity Clusters"""
        risk = abs(entry - sl)
        if self.direction == TradeAction.SELL:
    ...
    def generate_setup(self) -> TradeSetup:
        ...
        return TradeSetup(
            ...
            tp6=0.0,
            risk_reward=rr,
            fusion_score=0.85,  # akan diisi oleh main system
...
class PhantomGenesisSequential:
    """Menghubungkan Kerangka Berpikir → Logika Eksekusi dengan urutan rapi"""
    ...
    def run(self) -> Dict:
        # STEP 1: KERANGKA BERPIKIR (20 Layer + 5 Gates)
        thinker = ThinkingFramework(self.data)
        votes = thinker.get_all_votes()
        
        # Hitung Fusion Score (persentase bobot searah)
        ...
        # STEP 3: OUTPUT JSON sesuai format
        output = {
            ...
            "sl": round(setup.sl, 2),
            "tp": round(setup.tp1, 2),
            "confidence": round(setup.confidence, 2)
        }
        return output

# ============================================================
# 5. HELPER: JSONL APPEND (FIX #6) & DEX API (FIX #5)
# ============================================================

def append_jsonl(path: str, obj: Dict, max_retries: int = 3) -> bool:
    """
    FIX #6: Append to JSONL file safely (one object per line).
    Prevents memory issues and data corruption.
    """
    for attempt in range(max_retries):
        try:
            ...
            if attempt == max_retries - 1:
                logger.error(f"❌ Failed to append JSONL after {max_retries} attempts: {e}")
                return False
    ...
    for attempt in range(max_retries):
        try:
            ...
            if attempt == max_retries - 1:
                ...
                return None
            wait_time = 2 ** attempt
            logger.warning(f"⚠️ DEX API error (attempt {attempt + 1}/{max_retries}), retrying in {wait_time}s...")
            time.sleep(wait_time)
    return None

# ============================================================
# 6. CONTOH EKSEKUSI (Dengan Data Dummy + Real-Time)
# ============================================================

def create_dummy_data_with_realtime() -> MarketData:
    # Data Dasar (dari chart)
    ...
    base = MarketData(
        ...
        swing_low=4552.77,
        # Real-Time Data
        gex_zero_gamma=4567.00,
        gex_score=-0.82,
        cot_managed_money_net_long=90000,
        cot_bias="contrarian_bearish",
        order_book_bid_depth=450.0,
        order_book_ask_depth=850.0,
        vix=21.5,
        us10y=4.58
    )
    return base
...
if __name__ == "__main__":
    # FIX #2: Validate environment first
    try:
    ...
    # Ambil data pasar (dari MT5 + API Real-Time)
    ...
    # FIX #6: Example JSONL append (safe)
    append_jsonl("trades.jsonl", result)
